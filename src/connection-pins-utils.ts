...
export function getRelativeAnchor(
  ...
  const translatedRefPoint = translatePoint(
    refPoint,
    refContainer,
    connectable.parent
  );
  const strokeCorrection = 0.5 * connectable.strokeWidth;
  const anchor = anchorComputer.getAnchor(
    connectable,
    translatedRefPoint,
...
export function addConnectionPinsToShape(
  ...
  // - left
  let leftAnchor = getRelativeAnchor(
    child,
    { x: centerPoint.x - child.bounds.width, y: centerPoint.y },
    child.parent,
    anchorComputer
  );
  // TODO: check also y?
  const leftIsOnSide = Math.abs(leftAnchor.x) < 0.01;
  const shapeLeftPin = new Avoid.ShapeConnectionPin(
    shapeRef,
    1,
    leftIsOnSide ? 0 : leftAnchor.x,
    leftIsOnSide ? 0.5 : leftAnchor.y,
    leftIsOnSide,
    0,
    Directions.Left
  );
  ...
  // - right
  let rightAnchor = getRelativeAnchor(
    child,
    { x: centerPoint.x + child.bounds.width, y: centerPoint.y },
    child.parent,
    anchorComputer
  ...
  const shapeRightPin = new Avoid.ShapeConnectionPin(
    shapeRef,
    1,
    rightIsOnSide ? 1 : rightAnchor.x,
    rightIsOnSide ? 0.5 : rightAnchor.y,
    rightIsOnSide,
    0,
    Directions.Right
  );
  ...
  // - top
  let topAnchor = getRelativeAnchor(
    child,
    { x: centerPoint.x, y: centerPoint.y - child.bounds.height },
    child.parent,
    anchorComputer
  );
  // TODO: check also x?
  const topIsOnSide = Math.abs(topAnchor.y) < 0.01;
  const shapeTopPin = new Avoid.ShapeConnectionPin(
    shapeRef,
    1,
    topIsOnSide ? 0.5 : topAnchor.x,
    topIsOnSide ? 0 : topAnchor.y,
    topIsOnSide,
    0,
    Directions.Up
  );
  ...
  // - bottom
  let bottomAnchor = getRelativeAnchor(
    child,
    { x: centerPoint.x, y: centerPoint.y + child.bounds.height },
    child.parent,
    anchorComputer
  ...
  const shapeBottomPin = new Avoid.ShapeConnectionPin(
    shapeRef,
    1,
    bottomIsOnSide ? 0.5 : bottomAnchor.x,
    bottomIsOnSide ? 1 : bottomAnchor.y,
    bottomIsOnSide,
    0,
    Directions.Down
  );
...
export function updateConnPinsOnShapeResize(
  ...
  if (!shapeInfo.connPins.leftIsOnSide) {
    const leftAnchor = getRelativeAnchor(
      child,
      { x: centerPoint.x - child.bounds.width, y: centerPoint.y },
      child.parent,
      anchorComputer
    );
    shapeInfo.connPins.left.updatePosition(
      new Avoid.Point(leftAnchor.x, leftAnchor.y)
    );
  }
  ...
  if (!shapeInfo.connPins.rightIsOnSide) {
    ...
    shapeInfo.connPins.right.updatePosition(
      new Avoid.Point(rightAnchor.x, rightAnchor.y)
    );
  }
  ...
  if (!shapeInfo.connPins.topIsOnSide) {
    ...
    shapeInfo.connPins.top.updatePosition(
      new Avoid.Point(topAnchor.x, topAnchor.y)
    );
  }
  ...
  if (!shapeInfo.connPins.bottomIsOnSide) {
    ...
    shapeInfo.connPins.bottom.updatePosition(
      new Avoid.Point(bottomAnchor.x, bottomAnchor.y)
    );
  }
}
